...
def g2ToDict(folder_name,file_out_name,max_time,bin_width,pulse_spacing,max_pulse_distance,calc_norm=True,update=False,disp_counts=False, singles_channel_list=[3]):
    ...
    lib = ctypes.CDLL(working_directory + '/' + lib_name)
    if(disp_counts):
        lib.getG2Correlations_with_rate_calc.argtypes = [ctypes.c_char_p * num_files, ctypes.c_int, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_int64, ctypes.py_object, ctypes.POINTER(ctypes.c_int), ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.py_object, ctypes.c_int]
        start_time = time.time()
        #Call the DLL
        ...
        lib.getG2Correlations.argtypes = [ctypes.c_char_p * num_files, ctypes.c_int, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_int64, ctypes.py_object, ctypes.POINTER(ctypes.c_int), ctypes.c_int, ctypes.c_int, ctypes.c_int]
        start_time = time.time()
        #Call the DLL
        lib.getG2Correlations(ctypes_file_list, num_files, int_max_time, int_bin_width, int_pulse_spacing, max_pulse_distance, numer_list, ctypes.byref(denom_ctypes),calc_norm,4,4)
    print("Finished in " + str(time.time()-start_time) + "s")
...
def g2ToDict_pairwise(folder_name,file_out_name,max_time,bin_width,pulse_spacing,max_pulse_distance,calc_norm=True,update=False,disp_counts=False,channel_1_list=[0],channel_2_list=[1],singles_channel_list=[3]):
    ...
    if len(channel_1_list) == len(channel_2_list):
        ...
        int_pulse_spacing = round(pulse_spacing / int_bin_width) * int_bin_width
        max_bin  = int(round(int_max_time/int_bin_width))
        numer_list = [int(0)]*(2*max_bin + 1) * len(channel_1_list)
        denom_list = [int(0)] * len(channel_1_list)
        #Calculate what tau should look like
        tau = np.arange(-max_bin,max_bin+1) * int_bin_width
        ...
        if(disp_counts):
            ...
            start_time = time.time()
            #Call the DLL
            lib.getG2Correlations_pairwise(ctypes_file_list, num_files, int_max_time, int_bin_width, int_pulse_spacing, max_pulse_distance, numer_list, denom_list,calc_norm,4,4,channel_1_list, channel_2_list, len(channel_1_list))
        print("Finished in " + str(time.time()-start_time) + "s")
        ...
        if updating:
            try:
                #Try and add the newly calculated values to the old ones may fuck up if you ask for numerators of different sizes
                output_dict = {'numer_g2':np.reshape(np.array(numer_list),(len(channel_1_list),len(tau)))+old_numer,'denom_g2':denom_list+old_denom,'tau':tau,'file_list':dir_file_list}
            except:
                print("Could not update values")
        else:
            output_dict = {'numer_g2':np.reshape(np.array(numer_list),(len(channel_1_list),len(tau))),'denom_g2':denom_list,'tau':tau,'file_list':dir_file_list}
        return output_dict
    else:
        print("Channel lists are wrong size")
        return {}
...
def g3ToFile(folder_name,file_out_name,max_time,bin_width,pulse_spacing,max_pulse_distance,calc_norm=True):
    ...
    ctypes_file_list = file_list_to_ctypes(file_list, folder_name)
    lib = ctypes.CDLL(working_directory + '/' + lib_name)
    lib.getG3Correlations.argtypes = [ctypes.c_char_p * num_files, ctypes.c_int, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_int, ctypes.py_object, ctypes.POINTER(ctypes.c_int),ctypes.c_int,ctypes.c_int]
    start_time = time.time()
    #with wurlitzer.sys_pipes():
